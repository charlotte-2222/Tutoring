...
class VideoPlayer:
    def __init__(self, filename, volume=1.0):
        ...
        self.player = MediaPlayer(filename,
                                  ff_opts={'sync': 'audio', 'paused': False, 'volume': volume, 't': 1e7 + 1, 'ss': 0})
        time.sleep(1)
...
class ed(QWidget):
    def __init__(self, parent=None):
        ...
        pb = QPushButton('seek+');
        vl.addWidget(pb);
        pb.clicked.connect(self.seek_p)
        pb = QPushButton('seek-');
        vl.addWidget(pb);
        pb.clicked.connect(self.seek_m)
        pb = QPushButton('mute');
        vl.addWidget(pb);
        pb.clicked.connect(self.mute)
        pb = QPushButton('vol+');
        vl.addWidget(pb);
        pb.clicked.connect(self.vol_p)
        pb = QPushButton('vol-');
        vl.addWidget(pb);
        pb.clicked.connect(self.vol_m)
        self.l = QLabel();
        vl.addWidget(self.l)

    def play2(self):
        self.player = VideoPlayer("C:\\Users\\ayych\\Pictures\\Youtube\\piss123.mp4")
